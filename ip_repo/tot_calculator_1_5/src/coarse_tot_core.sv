...
always_ff @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    ...
    adc_samples_q <= adc_sample_vector_t'(sample);

    
    for (i=0; i<SAMPLE_NUM_PER_CYCLE; i++) begin

      // --- Rising edge ---
      if (
        !pulse_active &&
        (sample_prev < thr[11:0]) &&
        (adc_samples_q[i] >= thr[11:0])
      )
      begin
        rise_detected <= 1'b1;
        rise_prev_sample <= sample_prev;
        rise_curr_sample <= adc_samples_q[i];

        rise_coarse_time <= (coarse_counter * SAMPLE_NUM_PER_CYCLE) + i;
        pulse_active = 1'b1;
      end
      ...
      begin
        ...
        fall_curr_sample <= adc_samples_q[i];

        fall_coarse_time <= (coarse_counter * SAMPLE_NUM_PER_CYCLE) + i;
        pulse_active = 1'b0;
      end

      sample_prev <= adc_samples_q[i];
    end
  end
end
